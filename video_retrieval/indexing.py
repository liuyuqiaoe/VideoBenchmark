...
def indexing_s40a(start_idx, end_idx):
    ...
    batch_file_path = "/research/d7/fyp25/yqliu2/projects/VideoBenchmark/experiments/8_28_experiments/results/s40a/image_batches.json"
    paths_in_batch = get_image_batches(batch_file_path, start_idx, end_idx)
 
    db_path_root = os.path.join(os.getcwd(), "databases")
    root = "/research/d7/fyp25/yqliu2/projects/VideoBenchmark/experiments/9_11_experiments/results/vlm2vec_s40a_nopatches_indexing"
    progress_file = "s40a_completed_vlm2vec_nopatches.json"
    progress_file = os.path.join(root, progress_file)
    ...
    db_path = os.path.join(db_path_root, "s40a_vlm2vec_db")
    encoder = VLM2VecEncoder()
...
def indexing_s40a_image_patches(start_idx, end_idx):
    ...
    db_path_root = os.path.join(os.getcwd(), "databases")
    root = "/research/d7/fyp25/yqliu2/projects/VideoBenchmark/experiments/9_10_experiments/results/llava_s40a_17patches_indexing"
    progress_file = "s40a_completed_llava_17patches.json"
    progress_file = os.path.join(root, progress_file)
    ...
    encoder = LLaVAQwenEncoder() # i have set the flash-attn to be None in the loading function (rtx2080 doesn't support flash-attn)
    retriever = LanceDBVideoRetriever(encoder=encoder, db_path=db_path, table_name="image_17patches_embeddings")
...
def indexing_s40a_image_patches_vlm2vec(start_idx, end_idx):
    ...
    db_path_root = os.path.join(os.getcwd(), "databases")
    root = "/research/d7/fyp25/yqliu2/projects/VideoBenchmark/experiments/9_9_experiments/results/vlm2vec_s40a_5patches_indexing"
    progress_file = "s40a_completed_vlm2vec_5patches.json"
    progress_file = os.path.join(root, progress_file)
    ...
    db_path = os.path.join(db_path_root, "s40a_vlm2vec_db")
    encoder = VLM2VecEncoder() # the patches num can be adjusted in encoder
    retriever = LanceDBVideoRetriever(encoder=encoder, db_path=db_path, table_name="image_5patches_embeddings")
...
if __name__ == "__main__":
    # 74
    indexing_s40a_image_patches(16, 26)
    # indexing_s40a_image_patches_vlm2vec(2, 37)
    # indexing_s40a(2, 74)